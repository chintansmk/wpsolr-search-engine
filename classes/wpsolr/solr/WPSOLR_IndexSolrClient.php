...
class WPSOLR_IndexSolrClient extends WPSOLR_AbstractSolrClient {
	 ...
	 * @param $post
	 *
	 * @return WPSOLR_IndexSolrClient
	 */
	static function create_from_post( $post ) {

		// Get the current post language
		$post_language = apply_filters( WPSOLR_Filters::WPSOLR_FILTER_POST_LANGUAGE, null, $post );
	...
	public function index_data( $batch_size = 100, $post = null, $is_debug_indexing = false ) {
		...
		// Filter the query
		$query_statements = apply_filters( WPSOLR_Filters::WPSOLR_FILTER_SQL_QUERY_STATEMENT,
			array(
	...
	function create_solr_document_from_post_or_attachment(
		...
		$comments_con     = array();
		$comm             = isset( $this->solr_indexing_options[ WPSOLR_Schema::_FIELD_NAME_COMMENTS ] ) ? $this->solr_indexing_options[ WPSOLR_Schema::_FIELD_NAME_COMMENTS ] : '';
		...
		*/
		...
		$solarium_document_for_update[ WPSOLR_Schema::_FIELD_NAME_ID ]    = $pid;
		...
		// Remove HTML tags
		$solarium_document_for_update[ WPSOLR_Schema::_FIELD_NAME_CONTENT ] = strip_tags( $content_with_shortcodes_expanded_or_stripped );

		$solarium_document_for_update[ WPSOLR_Schema::_FIELD_NAME_AUTHOR ]             = $pauthor;
		$solarium_document_for_update[ WPSOLR_Schema::_FIELD_NAME_AUTHOR_S ]           = $pauthor_s;
		$solarium_document_for_update[ WPSOLR_Schema::_FIELD_NAME_TYPE ]               = $ptype;
		$solarium_document_for_update[ WPSOLR_Schema::_FIELD_NAME_DATE ]               = $pdate;
		$solarium_document_for_update[ WPSOLR_Schema::_FIELD_NAME_MODIFIED ]           = $pmodified;
		$solarium_document_for_update[ WPSOLR_Schema::_FIELD_NAME_DISPLAY_DATE ]       = $pdisplaydate;
		$solarium_document_for_update[ WPSOLR_Schema::_FIELD_NAME_DISPLAY_MODIFIED ]   = $pdisplaymodified;
		...
		if ( count( $custom_fields ) > 0 ) { // There are custom fields to index
			...
			if ( count( $post_custom_fields = get_post_custom( $post->ID ) ) ) { // There are custom fields to index on the post
				...
				if ( count( $existing_custom_fields ) > 0 ) {
					$solarium_document_for_update[ WPSOLR_Schema::_FIELD_NAME_CUSTOM_FIELDS ] = $existing_custom_fields;
				}
	...
	function extract_attachment_text_by_calling_solr_tika(
		...
		$attachment_text_extracted_from_tika = apply_filters( WPSOLR_Filters::WPSOLR_FILTER_ATTACHMENT_TEXT_EXTRACTED_BY_APACHE_TIKA, $attachment_text_extracted_from_tika, $solarium_extract_query, $post );
