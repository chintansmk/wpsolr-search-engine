		...
		<div class="wdm_row">
			<div class='col_left' style='width: 10%;'>
				...
				<select name='managed_solr_service_id'>
					<?php
					foreach ( WPSOLR_ManagedServers::get_managed_solr_services() as $list_managed_solr_service_id => $managed_solr_service ) {
						printf( "<option value='%s' %s>%s</option>",
							$list_managed_solr_service_id,
							selected( $list_managed_solr_service_id, $managed_solr_service_id, false ),
							$managed_solr_service[ WPSOLR_ManagedServers::MANAGED_SOLR_SERVICE_LABEL ]
						);
					}
					?>
				</select>
