...
$form_data                             = WPSOLR_Extensions::extract_form_data( $is_submit_button_form_temporary_index, array(
...
<div id="solr-hosting-tab">
	...
	<div id="solr-results-options" class="wdm-vertical-tabs-content">
		...
		if ( 'new_index' === $subtab ) {
			...
			if ( $option_object->can_add_another_index_type_temporary() ) {
				// No temporary index yet: display the form to create one.
				WPSOLR_Extensions::require_with( WPSOLR_Extensions::get_option_template_file( WPSOLR_Extensions::OPTION_MANAGED_SOLR_SERVERS, 'template-temporary-account-form.php' ),
					array(
						'managed_solr_service_id'   => $form_data['managed_solr_service_id']['value'],
						'response_error'            => ( isset( $response_object ) && ! WPSOLR_ManagedServers::is_response_ok( $response_object ) ) ? WPSOLR_ManagedServers::get_response_error_message( $response_object ) : '',
			...
			foreach ( ( isset( $option_data['solr_indexes'] ) ? $option_data['solr_indexes'] : array() ) as $index_indice => $index ) {
				...
				if ( $subtab === $index_indice ) {
					...
					if ( $is_index_type_temporary ) {
						...
						$managed_solr_server = new WPSOLR_ManagedServers( $option_object->get_index_managed_solr_service_id( $index ) );
						$response_object     = $managed_solr_server->call_rest_get_temporary_solr_index_status( $index_indice );

						if ( WPSOLR_ManagedServers::is_response_ok( $response_object ) ) {

							$is_index_unknown_on_server = WPSOLR_ManagedServers::get_response_result( $response_object, 'isUnknown' );
							...
							if ( $is_index_unknown_on_server ) {
								...
								$is_index_type_temporary_on_server = WPSOLR_ManagedServers::get_response_result( $response_object, 'isTemporary' );
				...
				<div
					...
					if ( $is_index_type_temporary ) {
						...
						<div class='col_right' style='width:90%'>
							...
							$OptionManagedSolrServer = new WPSOLR_ManagedServers( $managed_solr_service_id );
							foreach ( $OptionManagedSolrServer->generate_convert_orders_urls( $index_indice ) as $managed_solr_service_orders_url ) {
								...
								<input name="gotosolr_plan_yearly_trial"
								       ...
								       value="<?php echo $managed_solr_service_orders_url[ WPSOLR_ManagedServers::MANAGED_SOLR_SERVICE_ORDER_URL_BUTTON_LABEL ]; ?>"
								       onclick="window.open('<?php echo $managed_solr_service_orders_url[ WPSOLR_ManagedServers::MANAGED_SOLR_SERVICE_ORDER_URL_LINK ]; ?>', '__blank');"
								/>

								<?php


								//echo $managed_solr_service_orders_url[ WPSOLR_ManagedServers::MANAGED_SOLR_SERVICE_ORDER_URL_TEXT ];
