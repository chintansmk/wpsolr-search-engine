 ...
 */
class WPSOLR_Plugin_Wpml extends WPSOLR_Extensions {
	...
	protected function post_constructor() {
		...
		WPSOLR_Service_Wordpress::add_filter( WPSOLR_Filters::WPSOLR_FILTER_SQL_QUERY_STATEMENT, array(
			$this,
			'set_sql_query_statement',
		), 10, 2 );

		WPSOLR_Service_Wordpress::add_filter( WPSOLR_Filters::WPSOLR_FILTER_SEARCH_GET_DEFAULT_SOLR_INDEX_INDICE, array(
			$this,
			'get_default_solr_index_indice',
		), 10, 2 );


		WPSOLR_Service_Wordpress::add_filter( WPSOLR_Filters::WPSOLR_FILTER_SEARCH_PAGE_URL, array(
			$this,
			'set_search_page_url',
		), 10, 2 );


		WPSOLR_Service_Wordpress::add_filter( WPSOLR_Filters::WPSOLR_FILTER_POST_LANGUAGE, array(
			$this,
		...
		WPSOLR_Service_Wordpress::add_filter( WPSOLR_Filters::WPSOLR_FILTER_GET_CURRENT_LANGUAGE, array(
			$this,
	...
	public function output_form( $form_file = null, $plugin_parameters = [ ] ) {
		...
		parent::output_form(
			...
			array_merge(
				[
					'options'                               => WPSOLR_Global::getOption()->get_option_plugin_wpml(
						[ WPSOLR_Option::OPTION_SHARED_IS_EXTENSION_ACTIVE => '0' ]
					),
					'languages'                             => $this->get_languages(),
					'each_language_has_a_unique_solr_index' => $this->each_language_has_a_one_solr_index_search()
				],
				$plugin_parameters
			)
		);
	...
	function add_language_fields_to_document_for_update( $solarium_document_for_update, $solr_indexing_options, $post, $attachment_body ) {
		...
		if ( ! is_null( $post_language_code ) ) {
			...
			// Language field
			$solarium_document_for_update[ WPSOLR_Schema::_FIELD_NAME_LANGUAGE_CODE ] = $post_language_code;

			// Add fields for the language code
			foreach ( WPSOLR_Schema::$multi_language_fields as $field_definitions ) {
	...
	function get_languages() {
		...
		$languages = WPSOLR_Service_Wordpress::apply_filters( 'wpml_active_languages', null, 'orderby=id&order=desc' );
	...
	function get_solr_index_indices() {
		...
		return WPSOLR_Global::getOption()->get_plugin_wpml_solr_index_indices();
	}
	...
	public function set_query_keywords( $parameters ) {
		...
		$search_terms = $parameters[ WPSOLR_Filters::WPSOLR_ACTION_SOLARIUM_QUERY__PARAM_SEARCH_TERMS ];
		...
		$query->setQuery( $query_string === '' ? ( WPSOLR_Schema::_FIELD_NAME_DEFAULT_QUERY . ':' . $search_terms ) : $query_string );
	...
	function get_default_solr_index_indice( $solr_index_indice, $language_code ) {
		...
		if ( ! isset( $solr_indexes ) ) {
			// Languages not yet related to any Solr index search.
			throw new \Exception( sprintf( 'WPSOLR %s extension is activated, but not configured to match languages and Solr indexes.', self::get_option_plugin_name( $this->extension ) ) );
		}
		...
		foreach ( $solr_indexes as $solr_index_indice => $solr_index ) {
			...
			if ( isset( $solr_index['is_default_search'] ) && isset( $solr_index['indexing_language_code'] ) && ( $solr_index['indexing_language_code'] === $current_language_code ) ) {
				...
				// Is language a valid one ?
				if ( ! $this->is_language_code( $solr_index['indexing_language_code'] ) ) {
					throw new \Exception( sprintf( "WPSOLR %s extension is activated, but current language '%s' is not configured.", self::get_option_plugin_name( $this->extension ), $current_language_code ) );
				}
