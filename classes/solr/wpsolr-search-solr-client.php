...
class WPSolrSearchSolrClient extends WPSolrAbstractSolrClient {
	...
	public function set_solarium_query( WPSOLR_Query $wpsolr_query ) {
        ...
        //
        if ($wpsolr_query->get_wpsolr_query()  == ''){
            $this->set_keywords( $solarium_query, '*' );
        }else{
	 ...
	 */
	public function display_results( WPSOLR_Query $wpsolr_query, $results_display_css ) {

		$output        = array();
		$search_result = array();

        $is_list_display = false;
        if($results_display_css == "list-results"){
		...
		foreach ( $resultset as $document ) {
            ...
            //
            $vc_row_cls = "vc_col-sm-4";
            if($is_list_display){
                $vc_row_cls = "vc_col-sm-12";
            }

            $msg .= '<div class="wpb_column vc_column_container '.$vc_row_cls.'">';
            $msg .= '  <div class="vc_column-inner ">';
            $msg .= '    <div class="wpb_wrapper">';

            $msg .= "   <div id='res$i' class='result_row'>";
			...
			if ( isset($thumbnail_url) and $thumbnail_url != '') {
                ...
                if($is_list_display){
                    $place_img = 'http://placehold.it/120x75';
                }
			...
			//if ( false ) {
			//	$msg .= "<div class='p_content'><span class='res-desc'>$content - <a href='$url'>Content match</a> </span></div>";
			//} else {
			//	$msg .= "<div class='p_content'><span class='res-desc'>$content </span></div>";
			//}
			//if ( $comm_no == 1 ) {
            ...
            $msg .= '   </div> <!-- ./1st column ./wpb_wrapper -->';
            $msg .= ' </div> <!-- ./1st column ./vc_column-inner -->';
            $msg .= ' </div><!-- ./1st column -->';
